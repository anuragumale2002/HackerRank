...
void send_all_acceptable_packages(town* source, int source_office_index, town* target, int target_office_index) {    
    ...
    // Iterate through source office packages
    for (int i = 0; i < source_office->packages_count; i++) {
        if (source_office->packages[i].weight >= target_office->min_weight && source_office->packages[i].weight <= target_office->max_weight) {
            // Add package to target office
            target_office->packages = realloc(target_office->packages, sizeof(package) * (target_office->packages_count + 1));
            target_office->packages[target_office->packages_count++] = source_office->packages[i];
        } else {
            // Store rejected package temporarily
            temp_packages[temp_count++] = source_office->packages[i];
        }
    }

    // Update source office packages
